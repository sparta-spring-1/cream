...
---
### 6. 결제 및 정산 시스템
### PortOne 기반의 안전한 결제 프로세스
- **결제 무결성 보장 (2단계 검증)**:
  - 사전 준비(Prepare): 실제 결제 발생 전 거래 정보를 DB에 먼저 기록하여 결제 시도 데이터의 정합성을 보장합니다.
  - 사후 검증(Complete): 결제 완료 후 PortOne API를 통해 서버 간 결제 금액의 위변조 여부를 최종 검증합니다.
- **유연한 환불 관리**:
  - 판매자 및 관리자 권한에 따른 전체 환불 기능을 지원하며, PortOne API와 연동하여 취소 처리를 수행합니다.
  - PaymentHistory 및 Refund 엔티티를 통해 모든 결제 취소 및 환불 사유를 추적 관리합니다.
- **이벤트 기반 아키텍처**:
  - 결제 성공 시 PaymentCompletedEvent가 발행되며 알림 전송 등 부가적인 비즈니스 로직을 비동기적으로 처리합니다.

### 자동화된 정산 시스템 (Settlement Automation)
- **스케줄러 기반 정산 처리**:
  - Spring Scheduler를 활용해 매일 특정 시간에 정산 프로세스를 자동 실행합니다.
  - 23:00 (대기 설정): 당일 발생한 PAID_SUCCESS 상태의 결제 건을 추출하여 PENDING 상태의 정산 데이터를 생성합니다.
  - 13:00 (정산 완료): 전날 생성된 대기 건들을 최종 COMPLETED 상태로 전환하며 정산 처리를 마무리합니다.
- **결제-정산 상태 동기화**:
  - 결제가 취소되거나 환불될 경우, 연관된 정산 데이터의 상태도 즉시 REFUNDED로 업데이트하여 정산 오차를 방지합니다.
- **판매자 정산 대시보드**:
  - 판매자는 자신의 정산 내역을 페이징 목록으로 확인하고, 각 거래에 대한 상세 정산 정보를 조회할 수 있습니다.
...
- 프론트엔드는 S3의 정적 웹사이트 호스팅 기능을 통해 배포됩니다. 
### 4.트러블 슈팅 
